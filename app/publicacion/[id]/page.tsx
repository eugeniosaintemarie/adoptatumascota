...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!publicacion) {
    return {
      title: "Publicación no encontrada | Adopta Tu Mascota",
    }
  }

  const { mascota } = publicacion
  const ubicacionSinComa = publicacion.ubicacion.replace(",", "")
  const title = `${especieLabels[mascota.especie as Especie]} ${generoLabels[mascota.sexo as Sexo].toLowerCase()} ${razasLabels[mascota.raza as Raza]} en adopción en ${ubicacionSinComa}`
  const description = mascota.descripcion

  return {
    title: `${title} | Adopta Tu Mascota`,
    description,
    openGraph: {
      title,
      description,
